function [best_C, correspond_inacc, correspond_outacc, support_vec_num, support_vec_percentage] = LinearClassificationCV(X, Y, k1, k2, C)
...
% report the best hyperparameter chosen and its correspond accuracy
    ...
    % The outer cross validation
        ...
        fprintf('\nouterCV:outerFold:%d, C:%.3f, \nsvNum:%d(%.3f%%), estAcc:%.3f, testAcc:%.3f\n\n',i,C_best,sum(svInd),sum(svInd)/length(OuttrainX)*100,best_acc,clf_acc)
    end
end